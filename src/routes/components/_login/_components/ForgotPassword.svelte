<script>
	...
	import admiracionIcon from '../../../../assets/assets/registroIngresoMobile/admiracionIcon.png';
	export let view;
	...
	let contentFormFade = 'contentForm active forgotP';
...
<div transition:fade={{ duration: 800 }} class={loginFormFade}>
	...
	>
		<div class="contTitle forgotIcon">
			<img src={admiracionIcon} alt="" srcset="" />
		...
		<div>
			<span
				...
				<h2>@agua-456</h2>
				<h4 class="italic">Correo12334@gmail.com</h4>
			</span>
		</div>
		<div>
			<div>
				<label for="">Contraseña</label>
				<input bind:value={password} class="inputDark" type="password" autocomplete="false" />
			</div>
			<span class="red">
				La contraseña ingresada es incorrecta. <br /><b>¿Olvidaste tu contraseña?</b>
			</span>
		</div>
		<div>
			<button class="btnIniciar" on:click={() => enter()}>Iniciar</button>
			<label for="" class="bottomContent">¿Olvidaste tu contraseña?</label>
		</div>
	</div>
</div>
...
<style>
	...
	.contentForm.active.forgotP > div:nth-child(2) h1,
	.contentForm.active.forgotP > div:nth-child(2) h2,
	.contentForm.active.forgotP > div:nth-child(2) h4 {
		margin: 0;
		margin-bottom: 15px;
		text-align: left;
	}
	...
	.contentForm > div:nth-child(3) > div {
		display: flex;
		flex-direction: column;
		width: 100%;
	}
	.contentForm > div:nth-child(3) > span {
		text-align: left;
		display: block;
		position: relative;
		width: 100%;
	...
	@media only screen and (max-width: 425px) {
		...
		.loginForm .contentForm.forgotP > div:nth-child(2) {
			align-items: center;
			padding: 15px;
			width: calc(100% - 30px);
			position: relative;
		...
		.loginForm .contentForm.forgotP > div:nth-child(3) {
			height: auto;
			padding: 15px 30px;
			background: #ebebeb;
		}
